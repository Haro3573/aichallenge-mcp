...
2. Treat the returned compact summary as the sole conversational source of collection status. The complete normalized current result is delivered only to the app card as an ephemeral Markdown document; it is not a stored index or a comparison with earlier runs.
3. Return a compact Korean briefing with the collection time, `N/M` source success count, total item count, and only failed-source errors or warnings. Tell the user to use the app card's **Markdown 문서 다운로드** button for the full current result. Do not reproduce every item in the chat.
...
- Do not invent document contents that were not returned in the compact summary. For a source-specific follow-up that needs full native fields, call that source's `collect_<source_id>` tool.
